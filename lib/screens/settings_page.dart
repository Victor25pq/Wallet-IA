...
class _SettingsPageState extends State<SettingsPage> {
  ...
  void _showAddCategoryForm() {
    showModalBottomSheet<Category>(
      context: context,
      isScrollControlled: true,
      // Simplemente llamamos al formulario directamente
  ...
  // --- FUNCIÓN DE ELIMINAR (REFACTORIZADA) ---
  Future<void> _handleDeleteWallet(Account wallet) async {
    final index = _wallets.indexOf(wallet);
    setState(() => _wallets.remove(wallet)); // Actualización optimista

    try {
      await _supabaseService.deleteWallet(wallet.id);
    } catch (error) {
      // Si falla, restauramos el estado y mostramos un error
      if (mounted) {
        setState(() => _wallets.insert(index, wallet));
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al eliminar la billetera: $error')),
        );
      }
    }
  }
  ...
  Future<void> _handleDeleteCategory(Category category) async {
    ...
    setState(() => _categories.remove(category)); // Actualización optimista
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al eliminar la categoría: $error')),
        );
      }
    }
  }
